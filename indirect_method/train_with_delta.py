...
arm_name = sys.argv[3]
train_path = join('..', 'bilateral_free_space_sep_27', 'train', f'{arm_name}', data)
val_path = join('..', 'bilateral_free_space_sep_27', 'val', f'{arm_name}', data)
...
val_dataset = indirectDataset(val_path, window, SKIP, in_joints, is_rnn=is_rnn, filter_signal=f, return_prev_torque=True)
...
for e in range(epoch, epochs + 1):
    ...
    for i, (position, velocity, torque, jacobian, time, prev_torque) in enumerate(train_loader):
        position = position.to(device)
        velocity = velocity.to(device)
        torque = torque.to(device)
        prev_torque = prev_torque.to(device)
        if is_rnn: 
    ...
    if e % validate_each == 0:
        ...
        for i, (position, velocity, torque, jacobian, time, prev_torque) in enumerate(val_loader):
            position = position.to(device)
            velocity = velocity.to(device)
            torque = torque.to(device)
            prev_torque = prev_torque.to(device)
